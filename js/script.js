...
/* ============================================================ notice board */
async function renderNoticeBoard(site){
  let items = null;
  ...
  try{
    ...
    console.warn('お知らせ管理画面データの読み込みに失敗。内蔵データを表示します。', err);
  }

  // ② content.js内蔵のバックアップ（通常は空 = 何も表示しない）
  if (!items) items = (site.notices && site.notices.items) || [];

  paintNoticeBoard(items);
}

function paintNoticeBoard(items){
  const boardEl = q('[data-fill="notice-board"]');
  const listEl = q('[data-fill="notice-list"]');
  ...
  listEl.innerHTML = items.map(n => `
    <li class="notice-item level-${n.level === 'info' ? 'info' : 'urgent'}">
      <span class="notice-icon" aria-hidden="true">${n.level === 'info' ? 'ℹ' : '⚠'}</span>
      <div class="notice-body">
        <div class="notice-title">${escapeHTML(n.title)}</div>
        ${n.body ? `<div class="notice-text">${escapeHTML(n.body)}</div>` : ''}
      </div>
    </li>
  `).join('');
